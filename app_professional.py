...
def prepare_segment_input(recency, frequency, monetary):
    return pd.DataFrame([[recency, frequency, monetary]], columns=["Recency", "Frequency", "Monetary"])
...
def prepare_future_input(recency, frequency, monetary, total_items, avg_order_value, selected_features):
    ...
    return pd.DataFrame([[feature_map[c] for c in selected_features]], columns=selected_features)
...
def run_batch_predictions(df, scaler, kmeans, future_model, selected_features, segment_names):
    ...
    result_df["SegmentName"] = result_df["Cluster"].map(lambda x: segment_names.get(int(x), f"Segment {int(x)}"))
...
with st.sidebar:
    ...
    st.markdown('<p class="small-note">Run the notebook first so these files are available.</p>', unsafe_allow_html=True)
...
with dashboard_tab:
    ...
    st.write(
        "Upload a CSV file to generate portfolio-level analytics, customer segment insights, and future purchase predictions."
    )
    ...
    dashboard_file = st.file_uploader(
        ...
        help="Required columns: Recency, Frequency, Monetary, TotalItems, AvgOrderValue",
    )

    if dashboard_file is not None:
        df_dash = pd.read_csv(dashboard_file)
        missing_cols = [c for c in REQUIRED_COLUMNS if c not in df_dash.columns]

        if missing_cols:
            st.error(f"Missing required columns: {missing_cols}")
        else:
            result_dash = run_batch_predictions(
...
with batch_tab:
    ...
    st.write(
        ...
        "`Recency`, `Frequency`, `Monetary`, `TotalItems`, and `AvgOrderValue`."
    )
    ...
    if uploaded_file is not None:
        ...
        if missing_cols:
            ...
            result_df = run_batch_predictions(df, scaler, kmeans, future_model, selected_features, segment_names)
